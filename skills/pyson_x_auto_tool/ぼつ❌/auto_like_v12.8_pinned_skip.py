#!/usr/bin/env python3
"""
Project Prometheus v12.8 [Pinned Tweet Skip Edition]
固定ツイートをスキップし、時間順にいいね返しを実行する
"""
...
class PrometheusV12_8:
    ...
    def run(self):
        log_info("--- Project Prometheus v12.8 [Pinned Tweet Skip Edition] ---")
    ...
    def run_full_likeback_cycle(self, page):
        ...
        try:
            ...
            for _ in range(self.config.getint('Likeback', 'MAX_TWEET_SCROLLS', 5)):
                ...
                tweet_urls_on_page = []
                for tweet in tweets:
                    # --- v12.8 修正点: 固定ツイートをスキップ --- #
                    pinned_text_locator = tweet.locator("span:has-text('固定されたポスト')")
                    if pinned_text_locator.count() > 0:
                        log_info("固定ツイートを検出しました。スキップします。")
                        # 念のため、処理済みセットに追加して再処理を防ぐ
                        tweet_url_locator = tweet.locator("a[href*='/status/']").first
                        if tweet_url_locator.count() > 0:
                            processed_tweets.add(tweet_url_locator.get_attribute('href'))
                        continue
                ...
                if not tweet_urls_on_page:
                    log_info("このページのツイートは全て処理済みか、新しいツイートが見つかりません。")
                ...
                for tweet_url in tweet_urls_on_page:
                    ...
                    log_info(f"ツイートを処理: {full_tweet_url}")
        ...
        while scroll_attempts < max_scrolls:
            ...
            for cell in liker_cells:
                ...
                if user_handle_locator.count() > 0:
                    current_handles_on_page.add(user_handle_locator.first.inner_text())
