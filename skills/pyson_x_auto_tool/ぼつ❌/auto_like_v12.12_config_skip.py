#!/usr/bin/env python3
"""
Project Prometheus v12.12 [Config Skip Edition]
config.iniで指定された固定ツイートIDを確実にスキップし、タイムスタンプでソートする
"""
...
class Config:
    ...
    def create_default_config(self):
        log_info(f"デフォルトのconfig.iniを作成します: {self.config_path}")
        default_config = """
[General]
MY_USERNAME = your_x_username_here
SKIP_PINNED_TWEET_ID = 

[Likeback]
MAX_TWEET_SCROLLS = 5
MAX_LIKER_SCROLLS = 10
"""
        with open(self.config_path, 'w', encoding='utf-8') as f:
            f.write(default_config)
        log_error("config.iniにあなたのXユーザー名と、必要であれば固定ツイートIDを設定してください。")
        sys.exit(1)
...
class PrometheusV12_12:
    def __init__(self, config_path=None):
        ...
        self.skip_pinned_tweet_id = self.config.get('General', 'SKIP_PINNED_TWEET_ID', default=None)
    ...
    def run(self):
        log_info("--- Project Prometheus v12.12 [Config Skip Edition] ---")
            ...
            for _ in range(self.config.getint('Likeback', 'MAX_TWEET_SCROLLS', 5)):
                ...
                for tweet in tweets:
                    ...
                    if time_locator.count() > 0 and tweet_url_locator.count() > 0:
                        ...
                        # --- v12.12 修正点: 設定ファイルから固定ツイートIDをスキップ --- #
                        status_id = tweet_url.split('/')[-1]
                        if self.skip_pinned_tweet_id and status_id == self.skip_pinned_tweet_id:
                            log_info(f"設定で指定された固定ツイートをスキップします: {status_id}")
                            continue
