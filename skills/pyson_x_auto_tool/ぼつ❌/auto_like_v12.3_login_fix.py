#!/usr/bin/env python3
"""
Project Prometheus v12.3 [Login Fix Edition]
ログイン状態を確認してからプロフィールにアクセスするように修正
"""
...
class PrometheusV12_3:
    ...
    def run(self):
        log_info("--- Project Prometheus v12.3 [Login Fix Edition] ---")
    ...
    def check_and_perform_login(self, page):
        try:
            ...
            if "login" in page.url:
                ...
                log_info("=" * 50)
                input()
                # 再度ホームページにアクセスしてログインを確認
                page.goto("https://x.com/home", wait_until='domcontentloaded', timeout=30000)
                time.sleep(5)
                if "login" in page.url:
                    log_error("ログインが確認できませんでした。")
                    return False
