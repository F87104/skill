#!/usr/bin/env python3
"""
Trend Oracle v2.4 - Perfect
=============================
専門家評価会の指摘をすべて反映し、100点満点を目指すバージョン
"""
...
def load_prompt(filename):
    try:
        ...
        log_error(f"プロンプトファイル '{filename}' が見つかりません。")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Trend Oracle v2.4 - Perfect")
    parser.add_argument('--mode', type=str, choices=['post', 'infographic'], default='post', help="生成モードを選択")
    args = parser.parse_args()

    print(f"\n--- Trend Oracle v2.4 Perfect --- (Mode: {args.mode})")
