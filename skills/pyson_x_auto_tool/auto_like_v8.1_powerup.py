...
# =====================================================
# Project Prometheus v8.1 [Power-Up Edition]
# いいね量産・安定性・デバッグ機能を強化した改良版
# =====================================================
...
def log_debug(msg):
    print(f"{Colors.YELLOW}[{datetime.now().strftime('%H:%M:%S')}][DEBUG]{Colors.RESET} {msg}")
...
class Config:
    ...
    def getboolean(self, section, option, default=False):
        try: return self.config.getboolean(section, option, fallback=default)
        except: return default
...
class PrometheusPowerUp:
    ...
    def run(self):
        with sync_playwright() as p:
            # 初回の手動ログイン処理
            if not Path(self.user_data_dir).joinpath("Default").exists():
            ...
            try:
                while True:
                    ...
                    log_info(f"次のサイクルまで待機: {wait//60}分...")
                    time.sleep(wait)
    ...
    def run_cycle(self, page):
        try:
            ...
            log_debug(f"全力モード: {'ON' if powerful_mode else 'OFF'}")
            log_debug(f"ターゲットキーワード: {target_keywords}")
            ...
            for tweet in tweets:
                ...
                try:
                    ...
                    if user_locator.count() == 0: 
                        log_debug(f"ツイート{processed_tweets}: ユーザー名が見つかりません。スキップします。")
                        continue
                    user = user_locator.first.inner_text()
                    
                    if user.lower() in self.liked_users:
                        log_debug(f"ユーザー({user})は既にいいね済みです。スキップします。")
                        continue

                    text_locator = tweet.locator("[data-testid='tweetText']")
                    if text_locator.count() == 0: 
                        log_debug(f"ツイート{processed_tweets}: 本文が見つかりません。スキップします。")
                        continue
                    text = text_locator.inner_text()

                    should_like = False
                    if powerful_mode:
                        should_like = not any(k in text for k in exclude_keywords)
                        if not should_like: log_debug(f"ツイート{processed_tweets}: 除外キーワードに一致しました。")
                    else:
                        should_like = any(k in text for k in target_keywords) and not any(k in text for k in exclude_keywords)
                        if not should_like: log_debug(f"ツイート{processed_tweets}: ターゲットキーワードに一致しませんでした。")

                    if should_like and random.randint(1, 100) <= like_prob:
                        btn = tweet.locator("[data-testid='like']")
                        if btn.count() > 0:
                            btn.first.click()
                            log_success(f"いいね成功: {user} -> 「{text[:30].replace('\n', ' ')}...」")
                            self.liked_users[user.lower()] = datetime.now().isoformat()
                            self._save_liked_users()
                            liked_count += 1
                            time.sleep(random.randint(10, 20))
                        else:
                            log_debug(f"ツイート{processed_tweets}: いいねボタンが見つかりません。")
                except Exception as e:
                    log_debug(f"ツイート{processed_tweets}の処理中にエラー: {e}")
                    continue
            
            log_info(f"サイクル完了: {liked_count}件のいいねを実行しました。")
        except Exception as e:
            log_error(f"サイクル実行中に致命的なエラーが発生しました: {e}")

def main():
    log_info("Project Prometheus v8.1 [Power-Up Edition] 起動")
    app = PrometheusPowerUp()
    app.run()
